...
module Cubix.Language.Imp.Parametric.Full.Trans () where
#else
module Cubix.Language.Imp.Parametric.Full.Trans (
    translate
  , untranslate
  ) where
...
import qualified Language.Imp as Imp
...
do substs <- makeSubsts
   runCompTrans $ withSubstitutions substs $ deriveTrans origASTTypes (TH.ConT ''ImpTerm)

translate :: Imp.CompUnit -> ImpTerm CompUnitL
translate = trans

instance (Trans c l, Typeable l) => Trans [c] [l] where
  trans [] = riNilF
  trans (x:xs) = (trans x :: ImpTerm l) `iConsF` (trans xs)

instance (Trans c l, Typeable l) => Trans (Maybe c) (Maybe l) where
  trans Nothing  = riNothingF
  trans (Just x) = iJustF $ (trans x :: ImpTerm l)
...
do substs <- makeSubsts
   runCompTrans $ withSubstitutions substs $ deriveUntrans origASTTypes (TH.ConT ''ImpTerm)
